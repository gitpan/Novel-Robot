...
=head2 下载小说
    ...
    get_novel.pl -u [小说目录页url] -t [目标文件类型] -C [是否写目录] -S [是否显示进度条] -i [章节]
...
=head2 分段下载小说
    ...
    get_novel_split.pl -u [小说目录页url] -t [目标类型] -n [分段章节数] -C [是否写目录] -S [是否显示进度条]
...
=head2 转换小说格式
    ...
    convert_novel.pl -f [源文件] -t [目标文件类型(小写)] -w [作者] -b [书名]
...
=head1 ARG
    ...
    -F : max_floor_num, 贴子最大楼层数
    -I : {min/max}_{query/board/writer}_page, 结果列表只取 x-y 页
    -M : max_{query_item/board_tiezi/writer_novel}_num, 结果列表最多取x项
    -N : min_word_num , 贴子每层最小字数
    -P : password, wordpress 密码
    -R : remark, 一些补充说明
    -S : show_progress_bar, 显示进度条(默认显示)
    -T : tag，标签，例如 顾漫,小白
    -U : wordpress 用户
    -W : ...wordpress 地址
